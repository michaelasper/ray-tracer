        ...
        switch (t->kind()) {
            ...
            default:
                throw SyntaxErrorException(
                    "Expected: geometry, camera, or light information",
                    _tokenizer);
        }
    }
}
    ...
    for (;;) {
        ...
        switch (t->kind()) {
            ...
            case QUATERNIAN:
                ...
                scene->getCamera().setLook(quaternian[0], quaternian[1],
                                           quaternian[2], quaternian[3]);
                break;
            ...
            case RBRACE:
                ...
                if (hasViewDir) {
                    if (!hasUpDir)
                        throw SyntaxErrorException("Expected: 'updir'",
                                                   _tokenizer);
                    scene->getCamera().setLook(viewDir, upDir);
                } else {
                    if (hasUpDir)
                        throw SyntaxErrorException("Expected: 'viewdir'",
                                                   _tokenizer);
                }
            ...
            default:
                throw SyntaxErrorException("Expected: camera attribute",
                                           _tokenizer);
        }
    }
}

void Parser::parseTransformableElement(Scene* scene, TransformNode* transform,
                                       const Material& mat) {
    const Token* t = _tokenizer.Peek();
    switch (t->kind()) {
        case SPHERE:
        ...
        case LBRACE:
            ...
            break;
        default:
            throw SyntaxErrorException("Expected: transformable element",
                                       _tokenizer);
    }
}

// parse a group of geometry, i.e., enclosed in {} blocks.
void Parser::parseGroup(Scene* scene, TransformNode* transform,
                        const Material& mat) {
    ...
    for (;;) {
        ...
        switch (t->kind()) {
            ...
            case TRANSFORM:
            case LBRACE:
                parseTransformableElement(scene, transform,
                                          newMat.get() ? *newMat : mat);
                break;
            case RBRACE:
                _tokenizer.Read(RBRACE);
                return;
            ...
            default:
                throw SyntaxErrorException("Expected: '}' or geometry",
                                           _tokenizer);
        }
    }
}

void Parser::parseGeometry(Scene* scene, TransformNode* transform,
                           const Material& mat) {
    const Token* t = _tokenizer.Peek();
    switch (t->kind()) {
        case SPHERE:
...
void Parser::parseTranslate(Scene* scene, TransformNode* transform,
                            const Material& mat) {
    ...
    parseTransformableElement(
        scene, transform->createChild(glm::translate(glm::dvec3(x, y, z))),
        mat);
...
void Parser::parseRotate(Scene* scene, TransformNode* transform,
                         const Material& mat) {
    ...
    // Parse child geometry
    parseTransformableElement(
        scene, transform->createChild(glm::rotate(w, glm::dvec3(x, y, z))),
        mat);
...
void Parser::parseScale(Scene* scene, TransformNode* transform,
                        const Material& mat) {
    ...
    // Parse child geometry
    parseTransformableElement(
        scene, transform->createChild(glm::scale(glm::dvec3(x, y, z))), mat);
...
void Parser::parseTransform(Scene* scene, TransformNode* transform,
                            const Material& mat) {
    ...
    parseTransformableElement(scene,
                              transform->createChild(glm::transpose(
                                  glm::dmat4x4(row1, row2, row3, row4))),
                              mat);
...
void Parser::parseSphere(Scene* scene, TransformNode* transform,
                         const Material& mat) {
    Sphere* sphere = 0;
    Material* newMat = 0;
    ...
    for (;;) {
        ...
        switch (t->kind()) {
            ...
            case RBRACE:
                ...
                return;
            default:
                throw SyntaxErrorException("Expected: sphere attributes",
                                           _tokenizer);
        }
    }
}

void Parser::parseBox(Scene* scene, TransformNode* transform,
                      const Material& mat) {
    Box* box = 0;
    ...
    for (;;) {
        ...
        switch (t->kind()) {
            ...
            case RBRACE:
                ...
                return;
            default:
                throw SyntaxErrorException("Expected: box attributes",
                                           _tokenizer);
        }
    }
}

void Parser::parseSquare(Scene* scene, TransformNode* transform,
                         const Material& mat) {
    Square* square = 0;
    Material* newMat = 0;
    ...
    for (;;) {
        ...
        switch (t->kind()) {
            ...
            case RBRACE:
                ...
                return;
            default:
                throw SyntaxErrorException("Expected: square attributes",
                                           _tokenizer);
        }
    }
}

void Parser::parseCylinder(Scene* scene, TransformNode* transform,
                           const Material& mat) {
    Cylinder* cylinder = 0;
    Material* newMat = 0;
    ...
    for (;;) {
        ...
        switch (t->kind()) {
            ...
            case RBRACE:
                _tokenizer.Read(RBRACE);
                cylinder =
                    new Cylinder(scene, newMat ? newMat : new Material(mat));
                cylinder->setTransform(transform);
                scene->add(cylinder);
                return;
            default:
                throw SyntaxErrorException("Expected: cylinder attributes",
                                           _tokenizer);
        }
    }
}

void Parser::parseCone(Scene* scene, TransformNode* transform,
                       const Material& mat) {
    ...
    for (;;) {
        ...
        switch (t->kind()) {
            ...
            case RBRACE:
                ...
                return;
            default:
                throw SyntaxErrorException("Expected: cone attributes",
                                           _tokenizer);
        }
    }
}
    ...
    for (;;) {
        ...
        switch (t->kind()) {
            ...
            case MATERIALS:
                ...
                if (RPAREN != _tokenizer.Peek()->kind()) {
                    tmesh->addMaterial(
                        parseMaterial(scene, tmesh->getMaterial()));
                    for (;;) {
                        const Token* nextToken = _tokenizer.Peek();
                        if (RPAREN == nextToken->kind()) break;
                        _tokenizer.Read(COMMA);
            ...
            case RBRACE: {
                ...
                for (list<glm::dvec3>::const_iterator vitr = faces.begin();
                     vitr != faces.end(); vitr++) {
                    if (!tmesh->addFace((*vitr)[0], (*vitr)[1], (*vitr)[2])) {
                        ostringstream oss;
                        oss << "Bad face in trimesh: (" << (*vitr)[0] << ", "
                            << (*vitr)[1] << ", " << (*vitr)[2] << ")";
                        throw ParserException(oss.str());
                    }
                }

                if (generateNormals) tmesh->generateNormals();
            ...
            default:
                throw SyntaxErrorException("Expected: trimesh attributes",
                                           _tokenizer);
        }
    }
}
...
void Parser::parseFaces(list<glm::dvec3>& faces) {
    ...
    if (points.size() < 3)
        throw SyntaxErrorException("Faces must have at least 3 vertices.",
                                   _tokenizer);
            ...
            case RBRACE:
                ...
                if (!hasPosition)
                    throw SyntaxErrorException("Expected: 'position'",
                                               _tokenizer);
                _tokenizer.Read(RBRACE);
                return new PointLight(scene, position, color,
...
DirectionalLight* Parser::parseDirectionalLight(Scene* scene) {
    ...
    for (;;) {
        ...
        switch (t->kind()) {
            case DIRECTION:
                if (hasDirection)
                    throw SyntaxErrorException("Repeated 'direction' attribute",
                                               _tokenizer);
            ...
            default:
                throw SyntaxErrorException(
                    "expecting 'position' or 'color' attribute", _tokenizer);
        }
    }
}

// These ought to be done with template member functions, but compiler support
// for these is rather iffy...
...
Material* Parser::parseMaterialExpression(Scene* scene,
    ...
    for (;;) {
        ...
        switch (token->kind()) {
            ...
            case SPECULAR: {
                ...
                //        if( ! setReflective )
                //          mat->setReflective( specular );  // Default kr = ks
                //          if none specified
                break;
            }
            ...
            default:
                throw SyntaxErrorException("Expected: material attribute",
                                           _tokenizer);
        }
    }
}
