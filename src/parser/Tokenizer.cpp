...
unique_ptr<Token> Tokenizer::Get() { return unique_ptr<Token>(GetNext()); }
...
Token* Tokenizer::GetScalar() {
    // an INTEGER token
    string ret("");
    while (isdigit(CurrentCh) || '-' == CurrentCh || '.' == CurrentCh ||
           'e' == CurrentCh) {
        ret += CurrentCh;
        GetCh();
    }
...
unique_ptr<Token> Tokenizer::Read(SYMBOL kind) {
    unique_ptr<Token> T(Get());
    if (T->kind() != kind) {
