...
void ModelerCamera::calculateViewingTransformParameters() {
    ...
    if (fmod((float)mElevation, 2.0f * M_PI) < 3 * M_PI / 2 &&
        fmod((float)mElevation, 2.0f * M_PI) > M_PI / 2)
...
ModelerCamera::ModelerCamera() { reset(); }
...
void ModelerCamera::dragMouse(int x, int y) {
    ...
    switch (mCurrentMouseAction) {
        case kActionTranslate: {
            ...
            glm::vec3 transYAxis =
                glm::cross((mPosition - mLookAt), transXAxis);
