...
//------------------------------------- Help Functions
//--------------------------------------------
GraphicalUI* GraphicalUI::whoami(
    Fl_Menu_* o)  // from menu item back to UI itself
{
...
void GraphicalUI::cb_render(Fl_Widget* o, void* v) {
    ...
    if (pUI->raytracer->sceneLoaded()) {
        ...
        auto t_now = t_start;
        auto t_elapsed =
            std::chrono::duration<double, std::ratio<1>>(t_now - t_start)
                .count();
        ...
        if (pUI->aaSwitch() && !stopTrace) {
            ...
            while (!pUI->raytracer->checkRender()) {
                ...
                aaTime = now - aaStart;
                t_now = std::chrono::high_resolution_clock::now();
                t_elapsed = std::chrono::duration<double, std::ratio<1>>(
                                t_now - t_aaStart)
                                .count();
                t_total = std::chrono::duration<double, std::ratio<1>>(t_now -
                                                                       t_start)
                              .count();
                if ((now - prev) / CLOCKS_PER_SEC * 1000 >= intervalMS) {
                    print(buffer,
                          "Trace: %.2f, Aa: %.2f, Total: %.2f, aaRays: %d",
            ...
            print(buffer,
                  "Trace: %.2f, Aa: %.2f, Total: %.2f, Rays: %u, %u, %u",
                  t_trace, t_elapsed, t_total, imageRays, aaRays,
                  imageRays + aaRays);
            pUI->m_traceGlWindow->label(buffer);
            pUI->m_traceGlWindow->refresh();
        }
        /*
           ...
           min(100,(int)((double)(b_width*b_height*num_blocks)/(double)(width*height)*100.0));
                                                ...
                                                print(buffer, "%d%% of %s",
           frac, traceWindowLabel); pUI->m_traceGlWindow->label(buffer);
                                        }
                                        ...
                                        if (zOrder) {
                                                int mask = 1;
                                                for (int nn = 0; nn < bits;
           nn++) { ix += ((nextBlock & mask) >> nn); mask = mask << 1; iy +=
           ((nextBlock & mask) >> (nn + 1)); mask = mask << 1;
                                                }
                                                ix *= b_width;
                                                iy *= b_height;
                                                nextBlock++;
                                        ...
                                        // data has changed, update on next
           refresh pUI->m_debuggingWindow->m_debuggingView->setDirty();
        */
    }
}
...
Fl_Menu_Item GraphicalUI::menuitems[] = {
    ...
    {"&Load Cubemap...", FL_ALT + 'c',
     (Fl_Callback*)GraphicalUI::cb_load_cubemap},
...
GraphicalUI::GraphicalUI() : refreshInterval(10) {
    ...
    m_aaSamplesSlider = new Fl_Value_Slider(95, 205, 180, 20,
                                            "Pixel Samples\non Each Direction");
    m_aaSamplesSlider->user_data(
        (void*)(this));  // record self to be used by static callback functions
    ...
    m_aaThreshSlider = new Fl_Value_Slider(95, 237, 180, 20,
                                           "Supersample\nThreshold (x 0.001)");
    m_aaThreshSlider->user_data(
        (void*)(this));  // record self to be used by static callback functions
    ...
    m_leafSizeSlider =
        new Fl_Value_Slider(95, 309, 180, 20, "Target Leaf Size");
    m_leafSizeSlider->user_data(
        (void*)(this));  // record self to be used by static callback functions
