...
void Scene::glDraw(int quality, bool actualMaterials,
                   bool actualTextures) const {
    // try the non-bounded objects
    for (const auto& obj : objects) {
        obj->glDraw(quality, actualMaterials, actualTextures);
    }
}

void Geometry::glDraw(int quality, bool actualMaterials,
                      bool actualTextures) const {
    glPushMatrix();
    {
        // glm uses colunm major as default
        glm::dmat4x4 colMajor = transform->transform();
        glMultMatrixd(&colMajor[0][0]);
        glDrawLocal(quality, actualMaterials, actualTextures);
    }
    glPopMatrix();
}

void setMaterialProperty(GLenum property, glm::dvec3 value) {
    GLfloat val[4];
...
void setGLMaterial(const Material& mat, const SceneObject* object) {
    ...
    glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, (GLfloat)mat.shininess(i));
}

void SceneObject::glDraw(int quality, bool actualMaterials,
                         bool actualTextures) const {
    glPushMatrix();
    {
...
void Sphere::glDrawLocal(int quality, bool actualMaterials,
                         bool actualTextures) const {
...
void drawTesselatedSquare(int quality) {
    ...
    for (int j = 0; j < quality; j++) {
        ...
        for (int i = 0; i <= quality; i++) {
            ...
            glVertex3d(double(i) / double(quality), double(j) / double(quality),
                       0.0);
            glNormal3d(0.0, 0.0, 1.0);
            glVertex3d(double(i) / double(quality),
                       double(j + 1) / double(quality), 0.0);
        }
        glEnd();
    }
    glPopMatrix();
...
void Box::glDrawLocal(int quality, bool actualMaterials,
                      bool actualTextures) const {
    // Use this for display lists
    ...
    if (dispListItr == boxDisplayLists.end()) {
        dispListItr =
            (boxDisplayLists.insert(std::make_pair(quality, glGenLists(1))))
                .first;
        glNewList(dispListItr->second, GL_COMPILE);
...
void Cone::glDrawLocal(int quality, bool actualMaterials,
                       bool actualTextures) const {
...
void Cylinder::glDrawLocal(int quality, bool actualMaterials,
                           bool actualTextures) const {
    // Use this for display lists
    static std::map<int, GLuint> displayLists;
...
void Square::glDrawLocal(int quality, bool actualMaterials,
                         bool actualTextures) const {
    // Use this for display lists
    static std::map<int, GLuint> displayLists;
...
void Trimesh::glDrawLocal(int quality, bool actualMaterials,
    ...
    if (displayList == 0) {
        ...
        for (Faces::const_iterator itr = faces.begin(); itr != faces.end();
             ++itr) {
            ...
            if (normals.empty()) {
                ...
                // there exists some bad triangles such that two vertices
                // coincide check this before normalize
                if (glm::length(cv) > 0) glNormal3dv(&cv[0]);
            }

            if (!normals.empty()) glNormal3dv(&normals[vert1][0]);
            if (!materials.empty() && actualMaterials)
                setGLMaterial(*materials[vert1], *itr);
            glVertex3dv(&vertices[vert1][0]);

            if (!normals.empty()) glNormal3dv(&normals[vert2][0]);
            if (!materials.empty() && actualMaterials)
                setGLMaterial(*materials[vert2], *itr);
            glVertex3dv(&vertices[vert2][0]);

            if (!normals.empty()) glNormal3dv(&normals[vert3][0]);
            if (!materials.empty() && actualMaterials)
...
void DirectionalLight::glDraw() const {
    ...
    maxVec = glm::max(scene->getCamera().getEye(), maxVec);
    maxVec = glm::max(
        scene->getCamera().getEye() + scene->getCamera().getLook(), maxVec);
    maxDist = max(max(maxVec[0], maxVec[1]), maxVec[2]);

    glm::dvec3 uAxis = glm::normalize(orientation);

    // The first thing we need is the light's coordinate system (u,v,w).  To do
    // this, we will cross the light's orientation vector with the three
    // coordinate axes and find the 'best conditioned' one -- that is, the cross
    // product with the largest length (so we can normalize it w/o numerical
    // error).
    glm::dvec3 vAxis = glm::cross(uAxis, glm::dvec3(1.0, 0.0, 0.0));
    {
        glm::dvec3 test = glm::cross(uAxis, glm::dvec3(0.0, 1.0, 0.0));
        if (glm::length(test) > glm::length(vAxis)) vAxis = test;

        test = glm::cross(uAxis, glm::dvec3(0.0, 0.0, 1.0));
        if (glm::length(test) > glm::length(vAxis)) vAxis = test;
    }
