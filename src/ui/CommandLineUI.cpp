...
CommandLineUI::CommandLineUI(int argc, char** argv) : TraceUI() {
    ...
    string cubemap_file;
    while ((i = getopt(argc, argv, "tr:w:hj:c:z:d:a:s")) != EOF) {
        switch (i) {
            case 'r':
            ...
            case 'c':
                ...
                break;
            case 'z':
                m_nThreshold = atoi(optarg);
                break;
            case 'a':
                m_antiAlias = true;
                m_nSuperSamples = atoi(optarg);
            case 's':
                m_stochaticAlias = true;
            case 'd':
...
int CommandLineUI::run() {
    ...
    if (raytracer->sceneLoaded()) {
        ...
        if (buf) writeImage(imgName, width, height, buf);
...
void CommandLineUI::alert(const string& msg) { std::cerr << msg << std::endl; }
...
void CommandLineUI::usage() {
    ...
    cerr << "usage: " << progName << " [options] [input.ray output.png]" << endl
         ...
         << "  -c <FILE>   one Cubemap file, the remainings will be detected "
            "automatically"
         << endl;
}
