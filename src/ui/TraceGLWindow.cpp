// TraceGLWindow
// A subclass of FL_GL_Window that handles drawing the traced image to the
// screen
//
#include <iostream>
...
int TraceGLWindow::handle(int event) {
    ...
    if (event == FL_PUSH || event == FL_DRAG) {
        ...
        if (raytracer) {
            ...
            ((GraphicalUI*)traceUI)
                ->m_debuggingWindow->m_debuggingView->redraw();
            debugMode = false;
            refresh();
...
void TraceGLWindow::refresh() { redraw(); }
...
void TraceGLWindow::saveImage(char* iname) {
    ...
    raytracer->getBuffer(buf, m_nDrawWidth, m_nDrawHeight);
    if (buf) writeBMP(iname, m_nDrawWidth, m_nDrawHeight, buf);
}

void TraceGLWindow::setRayTracer(RayTracer* tracer) { raytracer = tracer; }
