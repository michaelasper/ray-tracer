...
void printString(const char *s) {
    ...
    if (!haveInitialized) makeRasterFont();
...
DebuggingView::~DebuggingView() { delete m_camera; }
        ...
        case FL_PUSH: {
            switch (eventButton) {
                case kMouseRotationButton:
                    m_camera->clickMouse(kActionRotate, eventCoordX,
                                         eventCoordY);
                    break;
                case kMouseTranslationButton:
                    m_camera->clickMouse(kActionTranslate, eventCoordX,
                                         eventCoordY);
                    break;
...
void DebuggingView::draw() {
    ...
    if (m_showCamera) drawCamera();
}

void DebuggingView::setRayTracer(RayTracer *tracer) { raytracer = tracer; }
...
void DebuggingView::drawRays() {
    ...
    for (std::vector<std::pair<ray *, isect *>>::const_iterator rayItr =
        ...
        if (m_showNormals) {
            glPushMatrix();
            glTranslatef((GLfloat)isectPoint[0], (GLfloat)isectPoint[1],
...
void DebuggingView::drawCamera() {
    ...
    glVertex3dv(&(-0.51 * sceneCamera.getU() - 0.51 * sceneCamera.getV())[0]);
    glVertex3dv(&(-0.51 * sceneCamera.getU() + 0.51 * sceneCamera.getV())[0]);
    glVertex3dv(&(0.51 * sceneCamera.getU() + 0.51 * sceneCamera.getV())[0]);
    glVertex3dv(&(0.51 * sceneCamera.getU() - 0.51 * sceneCamera.getV())[0]);
    glVertex3dv(&(-0.51 * sceneCamera.getU() - 0.51 * sceneCamera.getV())[0]);
