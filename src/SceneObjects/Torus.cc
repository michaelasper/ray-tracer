...
bool Torus::intersectLocal(ray& r, isect& i) const {
    ...
    double c_2 = 2.0 * dd * (pp - (a * a + b * b)) + 4.0 * pd * pd +
                 4.0 * a * a * d[2] * d[2];

    double c_1 = 4.0 * pd * ((pp) - (a * a + b * b)) + 8 * a * a * p[2] * d[2];
    double c_0 = (pp - (a * a + b * b)) * (pp - (a * a + b * b)) -
                 4.0 * a * a * (b * b - p[2] * p[2]);
    ...
    // Compute U and V
    double u = (1 - (glm::atan(P[2], P[0]) + glm::pi<double>()) /
                        (2 * glm::pi<double>()));
    double v = (glm::atan(P[1], sqrt(P[0] * P[0] + P[2] * P[2]) - a) +
                glm::pi<double>()) /
               (2 * glm::pi<double>());
