...
// defined in Windows.h However, there is some strange thing, the side of our
// structure is different from what it should though we define it in the same
// way as MS did. So, there is a hack, we use the hardcoded constanr, 14,
// instead of the sizeof to calculate the size of the structure. You are not
// supposed to worry about this part. However, I will appreciate if you find out
// the reason and let me know. Thanks.
//
...
std::vector<uint8_t> readBMP(const char* fname, int& width, int& height) {
    ...
    if ((file = fopen(fname, "rb")) == NULL) return std::vector<uint8_t>();
    ...
    if (bmih.biBitCount != 24) return std::vector<uint8_t>();
    /*
    ...
    */
    ...
    int foo = fread(image.data(), bytes, 1, file);
...
void writeBMP(const char* iname, int width, int height, const void* vdata) {
    ...
    bytes /= height;
    std::vector<unsigned char> scanline(bytes);
    for (int j = 0; j < height; ++j) {
        ...
        fwrite(scanline.data(), bytes, 1, foo);
    }
