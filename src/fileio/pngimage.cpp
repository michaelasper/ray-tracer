...
struct PNGReader {
    ...
    int init(const char *filename) {
        ...
        if ((infile = fopen(filename, "rb")) == NULL) return FOPEN_FAILURE;

        /* check that the file really is a PNG image; could
         * have used slightly more general png_sig_cmp() function instead */

        fread(sig, 1, 8, infile);
        if (png_sig_cmp(sig, 0, 8) != 0) return BAD_SIGNATURE;

        png_ptr =
            png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL);
        if (!png_ptr) return NO_MEMORY;
        ...
        png_read_info(png_ptr,
                      info_ptr); /* read all PNG info up to image data */
    ...
    uch *get_image(double display_exponent, int &pChannels, int &pRowbytes) {
        ...
        /* expand palette images to RGB, low-bit-depth grayscale images to 8
         * bits, transparency chunks to full alpha channel; strip
         * 16-bit-per-sample
         ...
         * where this file may have come from--so if it doesn't have a file
...
std::vector<uint8_t> readPNG(const char *fname, int &width, int &height) {
    PNGReader reader;
    if (reader.init(fname) != 0) return std::vector<uint8_t>();
    width = reader.width;
    ...
    if (!indata) return std::vector<uint8_t>();
    ...
    for (int j = 0; j < height; j++)
        for (int i = 0; i < rowBytes; i += channels)
            for (int k = 0; k < channels; k++)
                data[k + i + j * rowBytes] =
                    indata[k + i + (height - j - 1) * rowBytes];
