...
bool Geometry::intersect(ray& r, isect& i) const {
    ...
    glm::dvec3 dir =
        transform->globalToLocalCoords(r.getPosition() + r.getDirection()) -
        pos;
    ...
    if (intersectLocal(r, i)) {
        // Transform the intersection point & normal returned back into global
        // space.
...
bool Geometry::hasBoundingBoxCapability() const {
    ...
    // ComputeBoundingBox() or
    // the ComputeLocalBoundingBox() method must be implemented.

    // If no bounding box capability is supported for an object, that object
    // will be checked against every single ray drawn.  This should be avoided
    // whenever possible, but this possibility exists so that new primitives
    // will not have to have bounding boxes implemented for them.
    return false;
}
